...
namespace CoffeeShare.Infrastructure.Repositories
{
    public class CommentRepository : ICommentRepository
    {
        ...
        public async Task<List<Comment>> GetAllCommentsForRecipe(int recipeId)
            => await _context.Comments.Include(x => x.User).AsNoTracking().Include(x => x.Recipe).AsNoTracking().Where(x => x.RecipeId == recipeId).OrderByDescending(x => x.CreatedDate).ToListAsync();

        public async Task<Comment> GetCommentById(int id)
            => await _context.Comments.Include(x => x.User).AsNoTracking().Include(x => x.Recipe).AsNoTracking().SingleOrDefaultAsync(x => x.Id == id);
